...
int[] lm;
    ...
    public void performDivision(String temp, int r)
    {
        while (temp.length() > r)
        {
            String line = temp.substring(0, r);
        ...
        else if (temp.length() < r)
        {
            for (int i = temp.length(); i < r; i++)
                temp = temp + 'x';
            if(choice ==1){
            calLineMatrix(temp);
            multiplyLineByKey(temp.length());
    ...
    public int calDeterminant(int G[][], int K)
    {
        int resultOfDet;
        switch (K) {
            case 1:
                ...
                break;
            case 2:
                resultOfDet = G[0][0] * G[1][1] - G[1][0] * G[0][1];
                break;
            default:
                resultOfDet = 0;
                for (int j1 = 0; j1 < K; j1++)
                {
                    ...
                    resultOfDet += Math.pow(-1.0, 1.0 + j1 + 1.0) * G[0][j1]
                            * calDeterminant(m, K - 1);
                }   break;
        }
        return resultOfDet;
    }
     ...
     public boolean check(String key, int len)
    {
        ...
        if (d == 0)
        {
            System.out.println("Kunci tidak dapat dibalik");
            return false;
        }
    ...
    public static void main(String args[]) throws IOException
    {
        ...
        System.out.println("Masukkan nama file yang akan diproses : ");
        String line = in.readLine();
        System.out.println("Masukkan kunci : ");
        String key = in.readLine();
        double sq = Math.sqrt(key.length());
        if (sq != (long) sq)
            System.out.println("Tidak dapat membentuk matriks persegi");
        else
        {
            int size = (int) sq;
